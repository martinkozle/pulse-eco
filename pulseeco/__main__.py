...
def add_print_type_arg_json(
    parser: argparse._ActionsContainer,  # noqa: SLF001
) -> None:
    parser.add_argument("-J", "--json", action="store_true", help="print data as JSON")


def add_print_type_arg_table(
    parser: argparse._ActionsContainer,  # noqa: SLF001
) -> None:
    parser.add_argument(
        "-T", "--table", action="store_true", help="print data as a table"
    )
...
def add_sensors_subparser(
    subparsers: argparse._SubParsersAction,  # type: ignore[type-arg]  # noqa: SLF001
) -> None:
...
def add_sensor_subparser(
    subparsers: argparse._SubParsersAction,  # type: ignore[type-arg]  # noqa: SLF001
) -> None:
    parser_sensor = subparsers.add_parser("sensor", help="Get sensor info by ID")
    parser_sensor.set_defaults(func=sensor)
    parser_sensor.add_argument(
        "-s", "--sensor", help=help_strings["sensor"], type=str, required=True
    )
    add_print_type_arg_json(parser_sensor)


def add_common_args(
    parser: argparse.ArgumentParser, required_type: bool = False
) -> None:
    parser.add_argument(
        "-s", "--sensor", help=help_strings["sensor"], type=str, default="-1"
    )
    parser.add_argument(
        "-t", "--type", help=help_strings["type"], type=str, required=required_type
    )
    parser.add_argument("-f", "--from", help=help_strings["from_"], type=str)
    parser.add_argument("--to", help=help_strings["to"], type=str)


def add_data_raw_subparser(
    subparsers: argparse._SubParsersAction,  # type: ignore[type-arg]  # noqa: SLF001
) -> None:
    parser_data_raw = subparsers.add_parser("dataRaw", help="Get raw data")
...
def add_avg_data_subparser(
    subparsers: argparse._SubParsersAction,  # type: ignore[type-arg]  # noqa: SLF001
) -> None:
    parser_avg_data = subparsers.add_parser("avgData", help="Get average data")

    parser_avg_data.set_defaults(func=avg_data)
    parser_avg_data.add_argument(
        "-p", "--period", help=help_strings["period"], type=str
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PulseEco API client", prog="pulseeco")
    parser.add_argument(
        "-B",
        "--base-url",
        help="PulseEco API base URL",
        type=str,
        default="https://{city_name}.pulse.eco/rest/{end_point}",
    )
    parser.add_argument(
        "-U", "--user", help="PulseEco API user", type=str, required=True
    )
    parser.add_argument(
        "-P", "--password", help="PulseEco API password", type=str, required=True
    )
    parser.add_argument("-C", "--city", help="the city name", type=str, required=True)
...
def get_session(args: argparse.Namespace) -> PulseEcoAPI:
    return PulseEcoAPI(auth=(args.user, args.password), base_url=args.base_url)


def print_data(data: list[Any], args: argparse.Namespace) -> None:
    if args.json:
